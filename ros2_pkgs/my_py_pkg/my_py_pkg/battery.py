...
class BatteryNode(Node):
    ...
    def __init__(self):
        ...
        self.last_time_battery_state_change_ = self.get_current_time_seconds()
    ...
    def check_battery_state(self):
        ...
        if self.battery_state_ == "full":
            if time_now - self.last_time_battery_state_change_ > 4.0:
                self.battery_state_ = "empty"
                self.get_logger().info("Battery is emtpy! Charging battery...")
                self.last_time_battery_state_change_ = time_now
                self.callback_set_led_server(3,1)
        else:
            if time_now - self.last_time_battery_state_change_ > 6.0:
                self.battery_state_ = "full"
                self.get_logger().info("Battery is now full again.")
                self.last_time_battery_state_change_ = time_now
                self.callback_set_led_server(3,0)

    def callback_set_led_server(self, led_number, state):
    ...
    def callback_call_set_led(self, future, led_number, state):
        try:
            ...
            self.get_logger().info("Set Led service complete: " + str(response.success))
